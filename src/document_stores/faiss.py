...
class FAISSDocumentStore(SQLDocumentStore):
    ...
    ## Added to support retrieve in a batch ##
    def query_by_embedding_batch(
        ...
        """
        Batch version of `query_by_embedding` to handle multiple query embeddings at once.

        :param query_embs: A list or a 2D numpy array of query embeddings. Shape should be (num_queries, embedding_dim).
        :param filters: Optional filters to narrow down the search space for each query.
                        Example: {"name": ["some", "more"], "category": ["only_one"]}
        :param top_k: How many documents to return for each query
        :param index: Index name to query the document from.
        :param return_embedding: Whether to return the embeddings of the documents.
        :param headers: Optional headers to pass (not supported by FAISS)
        :param scale_score: Whether to scale the similarity score to the unit interval (range of [0,1]).
                            If true (default) similarity scores (e.g., cosine or dot_product) will be scaled to [0,1].
                            Otherwise raw similarity scores will be used.
        :return: A list of lists containing `Document` objects for each query.
        """
        if headers:
            raise NotImplementedError("FAISSDocumentStore does not support headers.")
        ...
        index = index or self.index
        if not self.faiss_indexes.get(index):
            raise Exception(f"Index named '{index}' does not exist. Use 'update_embeddings()' to create an index.")

        # Ensure `return_embedding` is set
        if return_embedding is None:
            return_embedding = self.return_embedding

        # Convert the query embeddings into a 2D numpy array (if it's a list of 1D arrays)
        if isinstance(query_embs, list):
            query_embs = np.vstack(query_embs).astype(np.float32)  # Shape: (num_queries, embedding_dim)
        else:
            query_embs = query_embs.astype(np.float32)

        # Handle filters (for now, no filtering is applied in FAISS, so this is a placeholder)
        if isinstance(filters, list):
            if len(filters) != len(query_embs):
                raise ValueError("Number of filters must match number of query embeddings.")
        else:
            filters = [filters] * len(query_embs)

        # If similarity is "cosine", normalize the embeddings
        if self.similarity == "cosine":
            self.normalize_embedding(query_embs)

        # Perform a batch search in FAISS
        score_matrix, vector_id_matrix = self.faiss_indexes[index].search(query_embs, top_k)

        # Prepare the final results for each query
        all_return_documents = []
        for query_idx, (scores, vector_ids) in enumerate(zip(score_matrix, vector_id_matrix)):
            # Convert vector IDs to string and filter out invalid IDs (-1)
            vector_ids_for_query = [str(vector_id) for vector_id in vector_ids if vector_id != -1]

            # Get the documents for these vector IDs
            documents = self.get_documents_by_vector_ids(vector_ids_for_query, index=index)

            # Assign the scores to each document
            scores_for_vector_ids = {str(v_id): s for v_id, s in zip(vector_ids, scores)}
            return_documents = []
            for doc in documents:
                ...
                # Optionally return the embedding for each document
                if return_embedding:
                    doc.embedding = self.faiss_indexes[index].reconstruct(int(doc.meta["vector_id"]))

                # Create a copy of the document to avoid modifications to the original object
                return_document = copy.copy(doc)
                return_documents.append(return_document)

            # Append the result for this query
            all_return_documents.append(return_documents)

        return all_return_documents
