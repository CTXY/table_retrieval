...
from .modeling.model.language_model import get_language_model, DPREncoder, DTREncoder
from .modeling.data_handler.data_silo import DataSilo, StreamDataSilo
from .modeling.data_handler.processor import TextSimilarityProcessor
from .modeling.model.biadaptive_model import BiAdaptiveModel
from .modeling.model.adaptive_model import SiameseModel
from .modeling.training.base import Trainer
...
class DensePassageRetriever(DenseRetriever):
    ...
    def __init__(
        ...
        query_structure: str = 'global',
        table_structure: str = 'global',
        ...
        **Example:**
        ...
        if isinstance(self.query_tokenizer, TapasTokenizer):
            ...
            self.query_encoder = DTREncoder(
                pretrained_model_name_or_path=query_embedding_model,
                model_type="TapasEncoder",
                use_auth_token=use_auth_token,
            )
        else:
            self.query_encoder = DPREncoder(
                ...
                use_auth_token=use_auth_token,
                encoder_structure=query_structure,
            )
        ...
        if isinstance(self.passage_tokenizer, TapasTokenizer):
            self.passage_encoder = DTREncoder(
                pretrained_model_name_or_path=passage_embedding_model,
                model_type="TapasEncoder",
                use_auth_token=use_auth_token,
            )
            ...
            if linearization == 'separator' and linearization_direction == 'row':
                n_added_tokens = 5
            elif linearization == 'separator' and linearization_direction == 'column':
                n_added_tokens = 4
            else:
                n_added_tokens = 0
            ...
            self.passage_encoder = DPREncoder(
                ...
                use_auth_token=use_auth_token,
                encoder_structure=table_structure,
        ...
        if linearization == 'separator' and linearization_direction == 'row':
            # add special tokens: 
            special_tokens = ["[Title]", "[Header]","[Rows]","[Row]","[sep]"]
            self.passage_tokenizer.add_tokens(special_tokens)
        elif linearization == 'separator' and linearization_direction == 'column':
            # add special tokens: 
            special_tokens = ["[Title]","[Columns]","[Column]","[sep]"]
            self.passage_tokenizer.add_tokens(special_tokens)
        ...
        self.processor = TextSimilarityProcessor(
            ...
            linearization_direction=linearization_direction,
...
class SiameseRetriever(DenseRetriever):
    ...
    def __init__(
        ...
        use_auth_token: Optional[Union[str, bool]] = None,
        scale_score: bool = True,
        default_path: str = "bert-base-uncased",
        structure: str = 'global',
        linearization: str = 'default',
        linearization_direction: str = 'row',
    ):
        ...
        if isinstance(self.tokenizer, TapasTokenizer):
            self.encoder = DTREncoder(
                pretrained_model_name_or_path=embedding_model,
                model_type="TapasEncoder",
                use_auth_token=use_auth_token,
            )
