...
from ...modeling.evaluation.eval import Evaluator, UniEvaluator
from ..model.biadaptive_model import BiAdaptiveModel
from ..model.adaptive_model import TripletModel, AdaptiveModel
...
class Trainer:
    ...
    def train(self):
        ...
        if hasattr(self.model, "language_model3"):
            ...
            assert len(self.data_silo.processor.query_tokenizer) == len(self.data_silo.processor.passage_tokenizer)
            self.model.verify_vocab_size(
                vocab_size=len(self.data_silo.processor.query_tokenizer),
            )
        elif (
        ...
        self.checkpoint_every = 100000 #  to be deleted
        ...
        #             data_loader=test_data_loader, tasks=self.data_silo.processor.tasks, device=self.device, siamese=self.siamese_model
        #         )
    ...
    def compute_loss(self, batch: dict, step: int) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            if isinstance(module, AdaptiveModel) and not self.siamese_model:
                logits = self.model.forward(
                    input_ids=batch["input_ids"], segment_ids=None, padding_mask=batch["padding_mask"]
                )
            elif isinstance(module, BiAdaptiveModel) or (isinstance(module, AdaptiveModel) and self.siamese_model):
                logits = self.model.forward(
                    query_input_ids=batch["query_input_ids"],
                    query_segment_ids=batch["query_segment_ids"],
                    query_attention_mask=batch["query_attention_mask"],
                    query_row_ids=batch["query_row_ids"],
                    query_col_ids=batch["query_col_ids"],
                    ...
                    passage_row_ids=batch["passage_row_ids"],
                    passage_col_ids=batch["passage_col_ids"],
                )
            elif isinstance(module, TripletModel):
                logits = self.model.forward(
                    query_input_ids=batch["query_input_ids"],
                    query_segment_ids=batch["query_segment_ids"],
                    query_attention_mask=batch["query_attention_mask"],
                    table_input_ids=batch["passage_input_ids"],
                    table_segment_ids=batch["passage_segment_ids"],
                    table_attention_mask=batch["passage_attention_mask"],
                    text_table_input_ids=batch["qt_input_ids"],
                    text_table_segment_ids=batch["qt_segment_ids"],
                    text_table_attention_mask=batch["qt_attention_mask"],
                )
            else:
                logits = self.model.forward(**batch)
            
            ## TODO: require to calculate two kinds of loss
        ...
        # print(f"Runtime of self.compute loss(): {use_time:.6f} seconds")
    ...
    def _load_checkpoint(cls, path: Path, data_silo: DataSilo, model, optimizer, local_rank: int = -1):
        ...
        trainer_checkpoint = torch.load(path / "trainer", map_location=map_location, weights_only=False)
        trainer_state_dict = trainer_checkpoint["trainer_state"]
        trainer_state_dict['checkpoint_root_dir'] = Path(trainer_state_dict['checkpoint_root_dir'])
        if local_rank != -1:
        ...
        reconstructed_numpy_rng_state = (numpy_rng_state[0], numpy.array(numpy_rng_state[1], dtype=numpy.uint8), *numpy_rng_state[2:])
        numpy.random.set_state(reconstructed_numpy_rng_state)
    ...
    def _save(self):
        ...
        serializable_numpy_rng_state = (
            numpy_rng_state[0], 
            numpy_rng_state[1].tolist(),  # Convert state array to list
            numpy_rng_state[2],  # position
            numpy_rng_state[3],  # has_gauss
            numpy_rng_state[4]  # cached_gaussian
        )
        ...
        torch.save(
            {
                ...
                "numpy_rng_state": serializable_numpy_rng_state,
...
class UniTrainer:
    ...
    def compute_loss(self, batch: dict, step: int) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            ## TODO: require to calculate two kinds of loss
            loss = self.model.logits_to_loss(logits=logits, global_step=self.global_step, **batch)
            # mlm_loss = self.model.logits_to_loss(logits=logits[1], global_step=self.global_step, **batch)
