...
from ..model.adaptive_model import TripletModel
...
class Evaluator:
    ...
    def __init__(self, data_loader: torch.utils.data.DataLoader, tasks, device: torch.device, report: bool = True, siamese: bool=False):
        """
    ...
    def eval(
        ...
        for batch in tqdm(self.data_loader, desc="Evaluating", mininterval=10):
            ...
            with torch.inference_mode():
                if isinstance(module, AdaptiveModel) and not self.siamese:
                ...
                elif isinstance(module, AdaptiveModel) and self.siamese:
                    logits = model.forward(  
                    ...
                    logits = model.forward(  # type: ignore [call-arg]   # type: ignore [call-arg]
                        ...
                        query_segment_ids=batch.get("query_segment_ids", None),
                        query_attention_mask=batch.get("query_attention_mask", None),
                        query_row_ids=batch.get("query_row_ids", None),
                        query_col_ids=batch.get("query_col_ids", None),
                        passage_input_ids=batch.get("passage_input_ids", None),
                        passage_segment_ids=batch.get("passage_segment_ids", None),
                        passage_attention_mask=batch.get("passage_attention_mask", None),
                        passage_row_ids=batch.get("passage_row_ids", None),
                        passage_col_ids=batch.get("passage_col_ids", None),
                    )
                else:
    ...
    def log_results(
        ...
        for head in results:
            ...
            print(section_header)  # Print section header

            for metric_name, metric_val in head.items():
                if metric_name not in ["preds", "labels"] and not metric_name.startswith("_"):
                    metric_line = f"{metric_name}: {metric_val}\n"
                    print(metric_line)  # Print metric line
                    # Optionally log to standard logger
                    if logging:
                        logger.info(metric_line)
                    # Track metrics if an external tracker is used (e.g., MLflow)
    ...
    #     base_path = os.sep.join(os.path.realpath(__file__).split(os.sep)[:-4])
    #     logs_directory = os.path.join(base_path, 'logs', dataset_name)
    #     os.makedirs(logs_directory, exist_ok=True)
    #     results_file = os.path.join(logs_directory, 'evaluation_results.txt')
    ...
    #                     # Optionally log to standard logger and tracker
    #                     if logging:
    #                         logger.info(metric_line)
    #                     # Track metrics if an external tracker is used (e.g., MLflow)
    #                     if isinstance(metric_val, numbers.Number):
    ...
    def eval(
        ...
        for batch in tqdm(self.data_loader, desc="Evaluating", mininterval=10):
            ...
            for head_num, head in enumerate(model.prediction_heads):
                loss_all[head_num] += np.sum(_to_numpy(losses_per_head[head_num]))
                for j in range(3):
                    preds_all[head_num] += list(_to_numpy(preds[head_num][j]))
                    label_all[head_num] += list(_to_numpy(labels[head_num][j]))
