...
from src.modeling.model.language_model import get_language_model, LanguageModel
...
class BiAdaptiveModel(nn.Module):
    ...
    def forward(
        ...
        pooled_output = self.forward_lm(
            ...
            passage_row_ids=passage_row_ids,
            passage_col_ids=passage_col_ids,
        )
    ...
    def forward_lm(
        ...
        if query_input_ids is not None and query_segment_ids is not None and query_attention_mask is not None:
            ...
            pooled_output1, _ = self.language_model1(
                input_ids=query_input_ids, segment_ids=query_segment_ids, attention_mask=query_attention_mask, row_ids=query_row_ids, col_ids=query_col_ids,
            )
            # language_model1_time = time.time() - start_time
            pooled_output[0] = pooled_output1
            # print(f"Runtime of self.language_model1(): {language_model1_time:.6f} seconds")

        if passage_input_ids is not None and passage_segment_ids is not None and passage_attention_mask is not None:
            # print("******************************")
            # print(passage_input_ids.shape)
            # print(passage_segment_ids.shape)
            # print(passage_attention_mask.shape)
            # print(passage_row_ids.shape)
            # print(passage_col_ids.shape)
            # start_time = time.time()
            max_seq_len = passage_input_ids.shape[-1]

            passage_input_ids = passage_input_ids.view(-1, max_seq_len)
            passage_col_ids = passage_col_ids.view(-1, max_seq_len)
            ...
            if len(passage_row_ids.shape) == 3:
                passage_row_ids = passage_row_ids.view(-1, max_seq_len)
            elif len(passage_row_ids.shape) == 4:
                passage_row_ids = passage_row_ids.view(-1, max_seq_len, max_seq_len)
            ...
            # max_values_across_batches_and_features = passage_segment_ids.max(dim=1)[0].max(dim=0)[0]

            # # 输出每个类型的最大值
            # for i in range(7):
            #     print(f"Type {i}: Max value = {max_values_across_batches_and_features[i]}")

            pooled_output2, _ = self.language_model2(
                input_ids=passage_input_ids, segment_ids=passage_segment_ids, attention_mask=passage_attention_mask, row_ids=passage_row_ids, col_ids=passage_col_ids,
            )
            # language_model2_time = time.time() - start_time
            pooled_output[1] = pooled_output2
            # print(f"Runtime of self.language_model2(): {language_model2_time:.6f} seconds")
