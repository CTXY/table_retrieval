...
from transformers.models.tapas.modeling_tapas import TapasConfig

from haystack.errors import ModelingError
from haystack.modeling.utils import silence_transformers_logs

from .modeling_dpr import DPRContextEncoder, DPRQuestionEncoder, BiasEncoder, EmbedEncoder
from .modeling_dtr import TapasEncoder, TapasForMaskedEncoder
...
class HFLanguageModel(LanguageModel):
    ...
    def __init__(
        ...
        * A local path of a model trained using Haystack (for example, "some_dir/haystack_model").
        ...
        if structure == 'bias' and model_type == 'Bert':
            model_class = MODEL_MAPPING['BiasEncoder']
        elif structure == 'auxemb' and model_type == 'Bert':
            model_class = MODEL_MAPPING['EmbedEncoder']
...
MODEL_MAPPING = {
    ...
    'BiasEncoder': BiasEncoder,
    'EmbedEncoder': EmbedEncoder,
}
...
class DPREncoder(LanguageModel):
    ...
    def __init__(
        ...
        use_auth_token: Optional[Union[str, bool]] = None,
        model_kwargs: Optional[Dict[str, Any]] = None,
        encoder_structure: Optional[str] = None,
    ):
        """
        Load a pretrained model by supplying one of the following:
        ...
        * A local path of a model trained using Haystack (for example, "some_dir/haystack_model").
        ...
        try:
            model_class = MODEL_MAPPING[model_classname]
        except AttributeError:
    ...
    def _init_model_haystack_style(
        ...
        if original_model_type and "dpr" in original_model_type.lower():
            dpr_config = DPRConfig.from_pretrained(haystack_lm_config, use_auth_token=use_auth_token)
            self.model = model_class.from_pretrained(
                haystack_lm_model, config=dpr_config, use_auth_token=use_auth_token, encoder_structure=self.encoder_structure, **model_kwargs
            )
            ...
            print(self.model.base_model.bert_model)
            
            # if self.encoder_structure == 'global':
            #     print('-----------------Instantiate the class for this model------------------')
            #     # Instantiate the class for this model
            #     print(language_model_class)
            #     self.model.base_model.bert_model = language_model_class(
            #         pretrained_model_name_or_path=model_name_or_path,
            #         model_type=original_model_type,
            #         use_auth_token=use_auth_token,
            ...
            #     ).model
            # else:
            #     self.model.base_model.bert_model.init_encoder(model_name_or_path, config=original_model_config, pretrained=True)
    ...
    def _init_model_transformers_style(
        ...
        """
        print('----------------------')
        print(model_name_or_path, model_class)
        ...
        if "dpr" in original_model_config.model_type.lower():
            ...
            print('load existing pretrained DPRQuestionEncoder model')
            self.model = model_class.from_pretrained(
                model_name_or_path, use_auth_token=use_auth_token, **model_kwargs
            )
        else:
            ...
            if self.encoder_structure in ['global', 'rowcol']:
                self.model.base_model.bert_model = AutoModel.from_pretrained(
                    model_name_or_path, use_auth_token=use_auth_token, **vars(original_model_config)
                )
            elif self.encoder_structure == 'bias':
                self.model.base_model.bert_model = BiasEncoder.from_pretrained(
                    model_name_or_path, use_auth_token=use_auth_token, **vars(original_model_config)
                )
                # self.model.base_model.bert_model.init_encoder(model_name_or_path, config=original_model_config, pretrained=False)
            elif self.encoder_structure == 'auxemb':
                self.model.base_model.bert_model = EmbedEncoder.from_pretrained(
                    model_name_or_path, use_auth_token=use_auth_token, **vars(original_model_config)
                )
    ...
    def _init_model_through_config(
        ...
        return model_class(config=DPRConfig(**config_dict), encoder_structure=self.encoder_structure)
    ...
    def save_config(self, save_dir: Union[Path, str]) -> None:
        ...
        setattr(DPRConfig, "model_type", self.model.config.model_type)
...
class DTREncoder(LanguageModel):
    """
    A DTREncoder model that wraps Hugging Face's implementation. 
    Support Tapas Model
    """
    ...
    def __init__(
        ...
        assert model_type == 'TapasEncoder'
        model_class = TapasEncoder

        haystack_lm_config = Path(pretrained_model_name_or_path) / "language_model_config.json"
        print('---------------------------------------------')
        print(haystack_lm_config)
        if os.path.exists(haystack_lm_config):
            print('---------------')
            self._init_model_haystack_style(
                haystack_lm_config=haystack_lm_config,
        ...
        if n_added_tokens != 0:
            ...
            print('------------Resizing embedding layer-----------')
            print(model_emb_size)
    ...
    def _init_model_haystack_style(
        ...
        print('----------------Line 899----------------------------')
        print(model_class, original_model_type)
        # if original_model_type and "dpr" in original_model_type.lower():
        model_config = AutoConfig.from_pretrained(haystack_lm_config, use_auth_token=use_auth_token)
        self.model = model_class.from_pretrained(
            haystack_lm_model, config=model_config, use_auth_token=use_auth_token, **model_kwargs
        )
        self.language = self.model.config.language
    ...
    def _init_model_transformers_style(
        ...
        model_config = AutoConfig.from_pretrained(model_name_or_path, use_auth_token=use_auth_token)
        
        self.model = model_class.from_pretrained(
            model_name_or_path, use_auth_token=use_auth_token, **model_kwargs
        )

        self.language = language or _guess_language(model_name_or_path)
    ...
    def save_config(self, save_dir: Union[Path, str]) -> None:
        ...
        :param save_dir: the path to save the model at
        """
        # For DPR models, transformers overwrites the model_type with the one set in TapasConfig
        # Therefore, we copy the model_type from the model config to TapasConfig
        setattr(TapasConfig, "model_type", self.model.config.model_type)
        super().save_config(save_dir=save_dir)
    ...
    def forward(
        ...
        return model_output.pooler_output, model_output.last_hidden_state


class UniEncoder(LanguageModel):
    """
    A UniEncoder model that wraps Hugging Face's implementation. 
    Support Tapas Model
    """
    ...
    def __init__(
        ...
        assert model_type == 'TapasForMaskedEncoder'
        model_class = TapasForMaskedEncoder
    ...
    def forward(
        ...
        return model_output.pooler_output, model_output.mlm_logits
