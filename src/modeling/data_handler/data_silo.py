...
import torch
from torch.utils.data import ConcatDataset, IterableDataset, Dataset, DataLoader
...
#     def _load_all_data(self, directory):
#         all_datasets = []
...
#             elif filename.startswith('problematic_ids_'):
#                 with open(os.path.join(directory, filename), 'rb') as f:
#                     all_problematic_ids.update(pickle.load(f))

#         return all_datasets, all_problematic_ids
...
#         num_dicts = len(dicts)
#         print(f'------------File Name: {filename}---------------Number of Dicts: {num_dicts}------------------------')
#         datasets = []
#         problematic_ids_all = set()
#         batch_size = self.max_multiprocessing_chunksize

#         save_interval = 100  # 每100个batch保存一次
#         current_count = 0  # 当前处理的批次数量
#         file_index = 0      # 文件索引，用于命名保存的文件
        
#         if str(self.processor.train_filename) in str(filename):
#             directory = '/home/yangchenyu/table_retrieval/corpus_data/nq_tables_tapas/train'
#         elif str(self.processor.dev_filename) in str(filename):
#             directory = '/home/yangchenyu/table_retrieval/corpus_data/nq_tables_tapas/dev'
        
#         if not os.listdir(directory):
#             for i in tqdm(range(0, num_dicts, batch_size), desc="Preprocessing dataset", unit=" Dicts"):
#                 # dicts[i]: query, passage
...
#                 datasets.append(dataset) # length of datasets + 1
#                 problematic_ids_all.update(problematic_sample_ids)

#                 if current_count >= save_interval:
#                     dataset_file_path = os.path.join(directory, f'dataset_{file_index}.pkl')
...
#             ids_file_path = os.path.join(directory, f'problematic_ids.pkl')
#             with open(ids_file_path, 'wb') as f:
#                 pickle.dump(problematic_ids_all, f)

#         else:
#             datasets, problematic_ids_all = self._load_all_data(directory)
#             tensor_names = ['query_input_ids', 'query_segment_ids', 'query_attention_mask', 'query_row_ids', 'query_col_ids', 'passage_input_ids', 'passage_segment_ids', 'passage_attention_mask', 'passage_row_ids', 'passage_col_ids', 'label_ids']
...
#             if "input_ids" in self.tensor_names and not self.siamese:
#                 clipped, ave_len, seq_lens, max_seq_len = self._calc_length_stats_single_encoder()
#             elif "input_ids" in self.tensor_names and self.siamese:
#                 clipped, ave_len, seq_lens, max_seq_len = self._calc_length_stats_siamese_encoder()
...
#     def _calc_length_stats_siamese_encoder(self):
#         seq_lens = []
#         for dataset in self.data["train"].datasets:
#             num_input = dataset[:][self.tensor_names.index("input_ids")].shape[1]
#             bs = dataset[:][self.tensor_names.index("input_ids")].shape[0]
#             all_input_numpy = (
#                 dataset[:][self.tensor_names.index("input_ids")].numpy().reshape((bs, -1), order="C")
#             )
#             input_len = np.sum(all_input_numpy != self.processor.tokenizer.pad_token_id, axis=1) / num_input
#             seq_lens.extend(input_len)
        
#         max_seq_len = dataset[:][self.tensor_names.index("input_ids")].shape[2]
#         clipped = np.mean(np.array(seq_lens) == max_seq_len) if seq_lens else 0
#         ave_len = np.mean(seq_lens) if seq_lens else 0
...
class StreamingDataset(IterableDataset):
    def __init__(self, processor, directory, batch_size):
        self.processor = processor
        self.directory = directory
        self.batch_size = batch_size
        self.file_list = [os.path.join(directory, f) for f in os.listdir(directory) if f.startswith('dataset_')]
        if 'train' in directory:
            self.dataset_size = 1350000
        elif 'dev' in directory:
    ...
    def __iter__(self):
        for file_path in self.file_list:
            with open(file_path, 'rb') as f:
                # Assuming each file contains a list of TensorDataset
                ...
                for idx in range(self.cumulative_sizes[-1]):
                    ...
                    yield datasets[dataset_idx][sample_idx]
...
class StreamDataSilo:
    ...
    def __init__(
        ...
        self.tensor_names = ['query_input_ids', 'query_segment_ids', 'query_attention_mask', 'query_row_ids', 'query_col_ids', 'passage_input_ids', 'passage_segment_ids', 'passage_attention_mask', 'passage_row_ids', 'passage_col_ids', 'label_ids']
        self.loaders = {}
    ...
    def _initialize_data_loaders(self):
        ...
        train_dataset = StreamingDataset(self.processor, '/home/yangchenyu/table_retrieval/corpus_data/pretraining/train', self.batch_size)
        
        dev_dataset = StreamingDataset(self.processor, '/home/yangchenyu/table_retrieval/corpus_data/pretraining/dev', self.eval_batch_size)
...
# # after obtaining propcessed data
# class DataSiloForUni:
#     """Generates and stores PyTorch DataLoader objects for the train, dev and test datasets.
#     Relies upon functionality in the processor to do the conversion of the data. Will also
#     calculate and display some statistics.
#     """
...
#         self.caching = caching
#         self.cache_path = cache_path
#         self.tensor_names = ['query_input_ids', 'query_segment_ids', 'query_attention_mask', 'query_labels', 'passage_input_ids', 'passage_segment_ids', 'passage_attention_mask', 'passage_labels', 'label_ids', 'qt_input_ids', 'qt_segment_ids', 'qt_attention_mask', 'qt_labels']
#         self.loaders = {}
...
#         train_dataset = StreamingDataset(self.processor, '/home/yangchenyu/table_retrieval/corpus_data/fetaqa_UTP/train', self.batch_size)
        
#         dev_dataset = StreamingDataset(self.processor, '/home/yangchenyu/table_retrieval/corpus_data/fetaqa_UTP/dev', self.eval_batch_size)
...
class DataSiloForUni:
    ...
    def _get_dataset(self, filename: Optional[Union[str, Path]], dicts: Optional[List[Dict]] = None):
        ...
        save_interval = 2  # 每100个batch保存一次
        current_count = 0  # 当前处理的批次数量
        file_index = 0      # 文件索引，用于命名保存的文件
        last_saved_index = 0  # 标记最后保存的数据的索引位置
        
        if str(self.processor.train_filename) in str(filename):
            directory = '/home/yangchenyu/table_retrieval/corpus_data/fetaqa_UTP/train'
        elif str(self.processor.dev_filename) in str(filename):
            directory = '/home/yangchenyu/table_retrieval/corpus_data/fetaqa_UTP/dev'
        ...
        if not os.listdir(directory):
            ...
            for i in tqdm(range(0, num_dicts, batch_size), desc="Preprocessing dataset", unit=" Dicts"):
                ...
                # print('*********************')
                # print(processing_batch[0]['query'])
                # print(len(processing_batch))
                # print(len(dataset))

                # print('---------------------------------------------')
                # print(type(dataset)) # TensorDataset
                # print(tensor_names) # ['query_input_ids', 'query_segment_ids', 'query_attention_mask', 'query_labels', 'passage_input_ids', 'passage_segment_ids', 'passage_attention_mask', 'passage_labels', 'label_ids', 'qt_input_ids', 'qt_segment_ids', 'qt_attention_mask', 'qt_labels']
                # print(type(problematic_sample_ids)) # set
                # print(problematic_sample_ids) # set()
                ...
                if current_count >= save_interval:
                    ...
                    # 保存自last_saved_index以来的新数据
                    print(f"-------------Save Data---------------")
                    with open(dataset_file_path, 'wb') as f:
                        pickle.dump(datasets[last_saved_index:], f)
                        
                    # 更新last_saved_index为当前datasets的长度，表示所有当前数据已被保存
                    last_saved_index = len(datasets)
                    current_count = 0
                    file_index += 1
            ...
            datasets, problematic_ids_all = self._load_all_data(directory)
            tensor_names = ['query_input_ids', 'query_segment_ids', 'query_attention_mask', 'query_labels', 'passage_input_ids', 'passage_segment_ids', 'passage_attention_mask', 'passage_labels', 'label_ids', 'qt_input_ids', 'qt_segment_ids', 'qt_attention_mask', 'qt_labels']
    ...
    def _calculate_statistics(self):
        ...
        if self.data["train"]:
            self.counts["train"] = len(self.data["train"])
            clipped, ave_len, seq_lens, max_seq_len = self._calc_length_stats_encoder()
        else:
            self.counts["train"] = 0
        ...
        if self.data["train"]:
            # SquadProcessor does not clip sequences, but splits them into multiple samples
            if "query_input_ids" in self.tensor_names and "passage_input_ids" in self.tensor_names and "qt_input_ids" in self.tensor_names:
                logger.info(
                    "Longest query length observed after clipping: %s   - for max_query_len: %s",
                    max(seq_lens[0]),
                    max_seq_len[0],
                ...
                logger.info("")
                logger.info(
                    "Longest query-table length observed after clipping: %s   - for max_qt_len: %s",
                    max(seq_lens[2]),
                    max_seq_len[2],
                )
                logger.info("Average query-table length after clipping:          %s", ave_len[2])
                logger.info("Proportion query-tables clipped:                    %s", clipped[2])
    ...
    def _calc_length_stats_encoder(self):
        seq_lens = [[], [], []]
        for dataset in self.data["train"].datasets:
            query_input_numpy = dataset[:][self.tensor_names.index("query_input_ids")].numpy()
            num_passages = dataset[:][self.tensor_names.index("passage_input_ids")].shape[1]
            ...
            qt_input_numpy = (
                dataset[:][self.tensor_names.index("qt_input_ids")].numpy().reshape((bs, -1), order="C")
            )

            qlen = np.sum(query_input_numpy != self.processor.tokenizer.pad_token_id, axis=1)
            plen = np.sum(passage_input_numpy != self.processor.tokenizer.pad_token_id, axis=1)
            qtlen = np.sum(qt_input_numpy != self.processor.tokenizer.pad_token_id, axis=1)

            seq_lens[0].extend(qlen)
            seq_lens[1].extend(plen)
            seq_lens[2].extend(qtlen)

        q_max_seq_len = dataset[:][self.tensor_names.index("query_input_ids")].shape[1]
        p_max_seq_len = dataset[:][self.tensor_names.index("passage_input_ids")].shape[2]
        qt_max_seq_len = dataset[:][self.tensor_names.index("qt_input_ids")].shape[2]

        clipped_q = np.mean(np.array(seq_lens[0]) == q_max_seq_len) if seq_lens[0] else 0
        ave_len_q = np.mean(seq_lens[0]) if seq_lens[0] else 0
        clipped_p = np.mean(np.array(seq_lens[1]) == p_max_seq_len) if seq_lens[1] else 0
        ave_len_p = np.mean(seq_lens[1]) if seq_lens[1] else 0
        clipped_qt = np.mean(np.array(seq_lens[2]) == qt_max_seq_len) if seq_lens[2] else 0
        ave_len_qt = np.mean(seq_lens[2]) if seq_lens[2] else 0

        clipped = [clipped_q, clipped_p, clipped_qt]
        ave_len = [ave_len_q, ave_len_p, ave_len_qt]
        max_seq_len = [q_max_seq_len, p_max_seq_len, qt_max_seq_len]
        return clipped, ave_len, seq_lens, max_seq_len
